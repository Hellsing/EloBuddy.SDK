...
namespace EloBuddy.SDK.Menu
{
    ...
    public sealed class ValueContainer : ControlContainer<ValueBase>
    {
            ...
            : base(ThemeManager.SpriteType.FormContentContainer, true, false, true)
        {
            ...
            if (addonId != null && serializationId != null && MainMenu.SavedValues != null && MainMenu.SavedValues.ContainsKey(AddonId) && MainMenu.SavedValues[AddonId].ContainsKey(serializationId))
            {
                //ChildrenSerializedData = new List<Dictionary<string, object>>(MainMenu.SavedValues[AddonId][serializationId]);
                foreach (var dictionary in MainMenu.SavedValues[AddonId][serializationId])
                {
                    if (dictionary != null)
                    {
                        ...
                        ChildrenSerializedData.Add(newDictionary);
                    }
                }
            }
        }
