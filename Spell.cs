...
namespace EloBuddy.SDK
{
    public static class Spell
    {
        ...
        public abstract class SpellBase : ISpell
        {
            ...
            public int ManaCost
            {
                get
                {
                    ...
                    return (int) Handle.SData.ManaCostArray[Level - 1];
                }
            }
            ...
            {
                return Player.Instance.GetSpellDamage(target, Slot);
            }
            ...
            {
                Circle.Draw(color.ToSharpDX(), Range, lineWidth, RangeCheckSource ?? Player.Instance.Position);
            }

            public virtual void DrawRange(Color color, float lineWidth = 3f)
            {
                Circle.Draw(color, Range, lineWidth, RangeCheckSource ?? Player.Instance.Position);
            }
        }
        ...
        public class Skillshot : Ranged
        {
            ...
            public Skillshot(SpellSlot slot, DamageType dmgType = DamageType.Mixed) : base(slot, 0, dmgType)
            {
                ...
                Range = (uint)spellInfo.Range;
                Speed = (int)spellInfo.MissileSpeed;
                Width = (int) spellInfo.Radius;
                CastDelay = (int)(spellInfo.Delay * 1000);
                ...
                switch (spellInfo.Type)
                {
                    ...
                    case SpellType.Cone:
                        ...
                        ConeAngleDegrees = (int)spellInfo.Radius;
                        break;
                    case SpellType.Arc:
                        Type = SkillShotType.Circular;
                        break;
            ...
            public virtual bool CastStartToEndAoe(Obj_AI_Base[] Entities, int HitCount, int StartToEndDistance, float HitChance = 60)
            {
                if (Entities == null)
                {
                    return false;
                ...
                var source = SourcePosition ?? Player.Instance.ServerPosition;

                var orderToSource = Entities.Where(o => o.IsValidTarget() && GetPrediction(o).HitChancePercent >= 60).OrderByDescending(o => o.Distance(source));
                var orderFromSource = Entities.Where(o => o.IsValidTarget() && GetPrediction(o).HitChancePercent >= 60).OrderBy(o => o.Distance(source));

                var results = new Dictionary<int, Geometry.Polygon.Rectangle>();

                foreach (var starts in orderToSource)
                {
                    var startPred = GetPrediction(starts).CastPosition;

                    foreach (var ends in orderFromSource)
                    {
                        var endPred = GetPrediction(ends).CastPosition;
                        var correctEnd = startPred.Extend(endPred, StartToEndDistance).To3D();

                        if (!this.PrecheckCast(correctEnd)) // skip
                        {
                            continue;
                        }

                        var rect = new Geometry.Polygon.Rectangle(startPred, correctEnd, Width);
                        var hitcount = Entities.Count(rect.IsInside);
                        results.Add(hitcount, rect);
                    }
                }
                ...
                var bestResult = results.OrderByDescending(r => r.Key).FirstOrDefault();

                if (bestResult.Key < HitCount)
                {
                    return false;
                }

                Vector3 start = bestResult.Value.Start.To3D();
                Vector3 end = bestResult.Value.End.To3D();
            ...
            //TODO Fix this when prediction supports linear AOE prediction
        ...
        public class SimpleSkillshot : SpellBase
        {
            ...
            /// <param name="dmgType">Damage type of the spell</param>
        ...
        public class Chargeable : Skillshot
        {
            ...
            {
                get { return Core.GameTickCount - ChargingStartedTime > FullyChargedTime && IsCharging; }
            }
