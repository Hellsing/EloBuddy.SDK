...
namespace EloBuddy.SDK
{
    public static class Orbwalker
    {
        ...
        internal static void TriggerAutoAttackReset(EventArgs args = null)
        {
            ...
            {
                NotifyEventListeners("OnAutoAttackReset", OnAutoAttackReset.GetInvocationList(), args);
            }
        }
        ...
        internal static void Initialize()
        {
            ...
            Obj_AI_Base.OnPlayAnimation += delegate (Obj_AI_Base sender, GameObjectPlayAnimationEventArgs args)
            {
                ...
                if (sender.IsMe && !args.Animation.Contains("Run") && !args.Animation.Contains("Idle"))
                {
                    Chat.Print(args.Animation);
                }
            };
        ...
        internal static void OnTick()
        {
            ...
            if (ActiveModesFlags != ActiveModes.None || DrawLastHitMarker || DrawDamageMarker)
            {
                ...
                TickCachedMinions.AddRange(EntityManager.MinionsAndMonsters.EnemyMinions.Where(o => Player.Instance.IsInRange(o, 1500)));
        ...
        internal static void OnUpdate()
        {
            if (Player.Instance.Spellbook.IsAutoAttacking)
            {
                ...
                if (castEndTime > 0)
                {
                    if (CanIssueOrder)
                    {
                        //Aa didn't get detected, so we trigger manually.
                        //_OnAttack(null);
                    }
